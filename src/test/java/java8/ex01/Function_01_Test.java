...
public class Function_01_Test {
    ...
    private Function<Integer, Person> intToPerson = iTp -> new Person("first_"+iTp, "last_"+iTp,iTp , "pass_"+iTp);
    // end::intToPerson[]
    ...
    private Function<Person, Account> personToAccount = pTa -> {
    	Account a = new Account();
    	a.setOwner(pTa);
    	a.setBalance(1000);
    	return a;
    };
    // Lambda simplifié par la déclaration du constructeur Account(Person,Integer)
    private Function<Person, Account> personToAccountBis = pTa -> new Account(pTa,1000);
    // end::personToAccount[]
    ...
    public void test_personToAccount() throws Exception {
        ...
        Account resultBis = personToAccountBis.apply(person);

        assert result.getOwner().equals(person);
        assert result.getBalance().equals(1000);
        
        assert resultBis.getOwner().equals(person);
        assert resultBis.getBalance().equals(1000);
    }
